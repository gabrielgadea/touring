...
// touring patch (14/09/2026): `lexer->lookahead` is a Unicode codepoint, and the
// <ctype.h> classifiers are defined only for `unsigned char` values — glibc
// indexes its ctype table with the argument, so `isdigit` on a high-plane
// character read past the table (SIGSEGV). See third_party/tree-sitter-bash/PATCHES.md.
