...
// touring patch: `lexer->lookahead` is a Unicode codepoint, and <ctype.h>
// classifiers are only defined for `unsigned char` values. `isdigit` on U+FF495
// indexed glibc's ctype table about 2 MB past its end (SIGSEGV under
// AddressSanitizer on an analise document, 14/09/2026). CommonMark list markers
// are ASCII digits only.
static inline bool touring_is_ascii_digit(int32_t c) { return c >= '0' && c <= '9'; }
#include <wctype.h>
...
// touring patch: the scanner counts open blocks in uint8_t (`matched`, the
// `(uint8_t)open_blocks.size` casts) and serializes 4 bytes per block into a
// 1024-byte buffer, so it is only sound below 256 blocks and 255 bytes of
// state. Past that the counters wrap, indexes run off the stack and memory is
// corrupted. Never track more than fits: a deeper block is not opened.
#define TOURING_MAX_OPEN_BLOCKS 254

static void push_block(Scanner *s, Block b) {
    if (s->open_blocks.size >= TOURING_MAX_OPEN_BLOCKS) {
        return;
    }
...
static inline Block pop_block(Scanner *s) {
    // touring patch: a refused push (see TOURING_MAX_OPEN_BLOCKS) can leave
    // the grammar closing a block the stack never held; never underflow.
...
static unsigned serialize(Scanner *s, char *buffer) {
    ...
    // touring patch: tree-sitter hands `buffer` with
    // TREE_SITTER_SERIALIZATION_BUFFER_SIZE (1024) bytes. 255 or more open
    // blocks wrote past it, into the TSParser that owns it, and a thread-reused
    // parser crashed later on another file. Keep only the outermost blocks
    // that fit; the parse of such a document degrades, memory stays intact.
    size_t max_blocks = (TREE_SITTER_SERIALIZATION_BUFFER_SIZE - size) / sizeof(Block);
...
static bool parse_ordered_list_marker(Scanner *s, TSLexer *lexer,
    ...
    if (s->indentation <= 3 &&
        ...
        size_t digits = 1;
        bool dont_interrupt = !touring_is_ascii_digit(lexer->lookahead);
        advance(s, lexer);
        while (touring_is_ascii_digit(lexer->lookahead)) {
            dont_interrupt = true;
            digits++;
            advance(s, lexer);
        }
