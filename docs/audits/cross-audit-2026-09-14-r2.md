...
2. **Conserto do wiring Python** pedido pela analise-08: **feito em 15/09** (`b4-wiring-python-2026-09-15.md`), na 30.4.52. Visibilidade de binding Python vem do nome, o arquivo passa a registrar o uso dos próprios símbolos, e a classe interno-only sai à parte. Medição viva no analise fica com a analise-08 depois do `touring update`. **B5 (16/09, decisão de Gabriel "Inclui relativo" + "Propagar 30.4.53 ao final")**: a analise-08 mediu os 231 órfãos que sobraram e nenhum estava sem consumidor — 61 eram from-import relativo (a consulta perdia os pontos), 70 uso qualificado `alias.Nome` (`import x` não escrevia consumidor algum) e 7 dunder. As três famílias foram corrigidas num ciclo só, com controle negativo ao lado de cada uma no teste de rebuild real e mutação 9/9. **Medido depois do deploy (geração 11 do índice): 231 → 91 no escopo, `novos_orfaos` = 0, from_import 61 → 0 e dunder 7 → 0 (ambos fechados), qualificado 70 → 53.** O qualificado fechou pouco porque a forma dominante no projeto real é `from pacote import modulo as alias` + `alias.Nome`, e `python_qualified_uses` só varre `import_statement` — lacuna de cobertura, distinta da exclusão deliberada de `import a.b`, com 71 ocorrências medidas entre os 53 sobreviventes; candidata a B6. *(Correção: este item dizia 136 e ~90; os valores medidos são 61 e 70, e as seis famílias somam 231 — propaguei o primeiro recado sem conferir contra o artefato.)*
