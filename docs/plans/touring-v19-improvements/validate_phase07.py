#!/usr/bin/env python3
"""
validate_phase07.py — Validation script for Phase 7: Final Audit and Cross-Validation
Plan: touring-v19-improvements

Runs all validation checks for phase 7 and exits 1 if any FAIL.
Usage: python validate_phase07.py [--verbose]
"""
import argparse
import shlex
import subprocess
...
def main() -> int:
    global VERBOSE
    parser = argparse.ArgumentParser(description="Validate Phase 7")
    parser.add_argument("--verbose", action="store_true")
    args = parser.parse_args()
    VERBOSE = args.verbose

    print(f"=== Phase 7 Validation: Final Audit and Cross-Validation ===")
    results = []
    ...
    results.append(check(
        "Checkpoint checkpoints/phase-07-final-audit.toon exists",
        lambda: (PLAN_DIR / "checkpoints/phase-07-final-audit.toon").exists()
    ))

    passed = sum(results)
    total = len(results)
    print(f"\n{'=' * 40}")
    print(f"Phase 7 Validation: {{passed}}/{{total}} PASS")
    if passed < total:
