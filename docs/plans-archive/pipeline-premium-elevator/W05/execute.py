#!/usr/bin/env python3
"""W05.execute — Wave W05.

Forensic sub-script for W05 — Wave W05.

Outputs
-------
  - data/W05-execute.json   (machine-readable JSON envelope)
- staging/W05-execute.md  (human-readable narrative)
Usage
-----
...
_RE_APEX_CACHE = __import__("re").compile(
    r"apex_phase_cache|APEX.*phase.*cache"
)
_RE_PRE_PHASE = __import__("re").compile(
    r"def\s+pre_phase\s*\(|hook\.pre_phase\s*\("
)
...
_RE_ERROR_HANDLING = __import__("re").compile(
    r"MemoryError|self._healing|retry.*phase"
)


def scan_workspace(_root: Path) -> list[dict]:
    """Scan pipeline_runner.py for PP4: run_phase() 5-responsibility extraction.

    PP4: Extract run_phase() CC=28 into 5 single-responsibility methods (CC≤4 each).
    ...
    # Check 1: should_run_phase() — phase skip logic
    ...
    findings.append({
        ...
        "pp": "PP4-execute",
        "context": (
            "should_run_phase() found"
            if should_run_match
            else "MISSING: should_run_phase() not found",
        ),
    })

    # Check 2: APEX phase cache
    apex_match = _RE_APEX_CACHE.search(content)
    findings.append({
        "file": "scripts/process_analysis/pipeline_runner.py",
        "line": line_for("apex_phase_cache") or line_for("APEX"),
        "severity": "PASS" if apex_match else "P2",
        "fc": "PP4",
        "pp": "PP4-execute",
        "context": (
            "APEX phase cache found"
            if apex_match
            else "WEAK: APEX phase cache not found",
        ),
    })

    # Check 3: pre_phase hook
    pre_phase_match = _RE_PRE_PHASE.search(content)
    findings.append({
        "file": "scripts/process_analysis/pipeline_runner.py",
        "line": line_for("hook.pre_phase("),
        "severity": "PASS" if pre_phase_match else "P1",
        "fc": "PP4",
        "pp": "PP4-execute",
        "context": (
            "pre_phase hook call found"
            if pre_phase_match
            else "MISSING: pre_phase hook not called",
        ),
    })

    # Check 4: _adapt_batch_size (memory adaptation)
    ...
    findings.append({
        ...
        "pp": "PP4-execute",
        "context": (
            "_adapt_batch_size() found"
            if adapt_match
            else "MISSING: _adapt_batch_size() not found",
        ),
    })

    # Check 5: _execute_phase
    ...
    findings.append({
        ...
        "pp": "PP4-execute",
        "context": (
            "_execute_phase() found"
            if exec_match
            else "MISSING: _execute_phase() not found",
        ),
    })

    # Check 6: Error handling (MemoryError + healing retry)
    ...
    findings.append({
        ...
        "pp": "PP4-execute",
        "context": (
            "Error handling (MemoryError/healing) found"
            if error_match
            else "WEAK: error handling pattern not detected",
        ),
    })
