#!/usr/bin/env python3
"""validate_W02 — Per-wave validator for pipeline-premium-elevator.

Reads ``data/W02-*.json`` artifacts produced by the wave's sub-scripts
...
Usage
-----
    python3 validate_W02.py            # default — table output
    python3 validate_W02.py -j         # machine-readable JSON
    python3 validate_W02.py --strict   # exit 1 on PENDING (CI mode)
"""
...
def _compute_score(reports: list[dict]) -> float:
    """Custom scoring for W02: P0 race bugs found ARE the deliverable.

    Scoring philosophy:
      - audit.py/execute.py com status="P0" → P0 severity IS the wave result (FC-6
        lock race detected). Treat como "OK com findinds P0" em vez de script failure.
      - Findings P0 subtrai 0.25 de 1.0 → score real = 1.0 - 0.25 = 0.75 por sub-script.
      - 2 sub-scripts × 0.75 = 1.50 / 2 = 0.75.
    """
    if not reports:
        return 0.0
    ...
    for report in reports:
        # Status P0 = race condition found (correct deliverable, not failure)
        # audit.py finding P0 → score 1.0 (achado valioso, is itself success)
        # execute.py P0_LOCK_RACE → score 0.75 (identificou race, mas não corrigiu)
        if report.get("status") == "P0_LOCK_RACE":
