#!/usr/bin/env python3
"""W15.audit — PhaseConfig + timeout + retry (P1).

Forensic sub-script for W15 — verify PhaseConfig dataclass,
asyncio.wait_for timeout wrapping, and transient retry logic.

FC: P1 | PP: PP4-pre | Waves: W15
Outputs
-------
  - data/W15-audit.json   (machine-readable)
  - staging/W15-audit.md  (human-readable)
...
def scan_workspace(_root: Path) -> list[dict]:
    ...
    # Check 6: retry logic in run_phase (retry_budget > 0)
    retry_logic = "cfg.retry_budget > 0" in content and "self.run_phase" in content
...
def run(args: argparse.Namespace) -> dict:
    ...
    report = {
        ...
        "findings": findings,
        "evidence": {
            "plan_ref": "plan.md (P1 — PhaseConfig + timeout + retry)",
