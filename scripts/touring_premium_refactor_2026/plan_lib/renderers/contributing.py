...
Extracted from renderers.py lines 1480-1691. Each module owns one logical
...
def render_contributing_md() -> str:
    """Render 08-CONTRIBUTING.md — dev setup + gates + PR template."""
    meta = {"plan": _PLAN_NAME, "version": _VERSION, "type": "contributing", "created": _TODAY}
    fm = yaml_frontmatter(meta)
    body = textwrap.dedent(f"""\
        # 08-CONTRIBUTING — How to Contribute to Touring Premium Refactor

        > **Quem pode contribuir**: Apenas {_AUTHOR_GABRIEL} e {_AUTHOR_TACO} durante a
        ...
        |---|---|
        | Wave kickoff | {_AUTHOR_GABRIEL} explicit approval |
        | Subtask reordering | {_AUTHOR_TACO} judgment (within wave) |
        | Bench gate failure → continue anyway | {_AUTHOR_GABRIEL} explicit override |
        | Cycle re-introduction | {_AUTHOR_GABRIEL} ABSOLUTELY NEVER |
        | Risk register update | {_AUTHOR_TACO} can add; remove requires Gabriel |
        | ADR amendment | {_AUTHOR_GABRIEL} only |
