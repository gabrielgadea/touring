...
**Every authored phase carries a ticket**: `touring decompose ticket <task> <subtask> --kind decision|implementation --fog clear|hazy|unknown --origin-ticket <id>`, and the finished plan is checked with `touring decompose frontier <task>`, which separates OPEN DECISIONS from the implementation they gate. A plan that schedules implementation under an open decision plans as if the unknown were resolved; fog that was never evaluated is `unknown`, never `clear`. Ref: `Touring/references/skill-operating-principles.md#p5`.
...
The Mermaid DAG is the plan's **flat, inspectable graph** — the same contract
`touring adw explain <name>` provides for a flow: what will actually run, after
composition, readable BEFORE anything runs. Two consequences for an authored plan:
every phase marked `parallel` declares its failure policy (`all|any|ignore|
best_effort|quorum:N` — an undeclared policy is silently `all`, so one flaky branch
sinks the phase), and every phase declares `when_not_to_use`, the field that lets a
reader DISCARD it rather than pick the nearest-looking one. Ref: `Touring/references/skill-operating-principles.md` (P4).
