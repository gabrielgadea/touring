...
/// Return names of any `expected` tables that are absent from `conn`.
///
/// Shared by all three `validate_*_tables` functions; each caller supplies
/// the slice of table names it owns.
fn check_tables(conn: &rusqlite::Connection, expected: &[&'static str]) -> Vec<&'static str> {
...
mod tests {
    ...
    fn test_wiring_map_has_correct_columns() {
        ...
        conn.execute_batch(touring_foundation::schema::knowledge::KNOWLEDGE_SCHEMA_V8)
            .expect("apply V8 schema");
        // Verify the operational column names exist
        conn.execute(
    ...
    fn test_validate_graph_tables_partial() {
        let conn = rusqlite::Connection::open_in_memory().expect("open in-memory");
        // Create only 2 of the 4 expected tables.
        conn.execute_batch(
