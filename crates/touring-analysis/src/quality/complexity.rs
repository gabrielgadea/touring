...
pub fn estimate_complexity(source: &str, language: &str) -> ComplexityMetrics {
    ...
    let branch_keywords: Vec<&[u8]> = match language {
        ...
        l if is_shell_language(l) => {
            vec![b"if ", b"elif ", b"for ", b"while ", b"until ", b"case "]
        }
...
fn count_lines(source: &str, language: &str) -> (usize, usize, usize) {
    let python_family = matches!(language, "python" | "py" | "ruby") || is_shell_language(language);
    let mut sloc = 0usize;
    let mut cloc = 0usize;
...
"#;
    ...
    fn a_shell_script_counts_its_functions_and_hash_comments() {
        ...
        for dialect in ["shell", "bash", "sh", "zsh"] {
            ...
            assert_eq!(
                m.function_count, 4,
                "{dialect}: `function a() {{` counts once"
            );
        ...
        assert_eq!(
            estimate_complexity("# setup() {\n", "shell").function_count,
            0
        );
    }
}
