...
fn parse_cpu_range_list(s: &str) -> Result<Vec<u32>, TopologyError> {
    ...
    for part in s.split(',') {
        ...
        if let Some((start, end)) = part.split_once('-') {
            ...
            let cpu = part.parse::<u32>().map_err(|_| TopologyError::ParseError {
                cpu_id: 0,
                field: "online_single",
...
pub fn detect_topology() -> Result<TopologyMap, TopologyError> {
    ...
    for &cpu_id in &cpus {
        match read_cluster_id(cpu_id) {
            ...
            Err(e) => {
                tracing::warn!(
                    cpu_id, error = % e, "failed to read cluster_id, skipping cpu"
                );
            }
        }
    }
