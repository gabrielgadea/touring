...
impl SlaTracker {
    /// Start tracking elapsed time. The tier is supplied later via the
    /// [`SlaSpec`] passed to [`SlaTracker::finish`], which is the single source
    /// of truth for the violated tier — so the tracker holds no redundant tier.
    pub fn start() -> Self {
        Self {
            start: Instant::now(),
        }
