...
use crate::sentinel::memory::{snapshot, MemorySnapshot, Pressure, SwapState};
...
impl MemoryGuard {
    ...
    pub async fn start_ticker(&self, interval: Duration) -> Result<(), TrmError> {
        // Verify Tokio runtime is active (gives a clean error vs. panic).
        ...
        let join = tokio::spawn(async move {
            // Floor interval to 100 ms to avoid pathological busy-loops.
...
mod tests {
    ...
    fn pressure_u8_roundtrip() {
        ...
        // Invalid values map to Green (defensive).
    ...
    fn snapshot_is_none_before_first_tick() {
        // global() may have ticked from another test — this test is sloppy but
        // documents the documented behavior: snapshot() may return None.
    ...
    async fn start_ticker_returns_already_running_on_second_call() {
        ...
        // First call may succeed or fail depending on test order — try both.
        let _ = g.start_ticker(Duration::from_millis(100)).await;
        // Second call must return TickerAlreadyRunning.
    ...
    async fn ticker_updates_pressure_within_two_ticks() {
        // Use a fresh non-global guard via direct construction.
        let g = MemoryGuard::new();
        // start_ticker requires a runtime — current_thread provides one.
        g.start_ticker(Duration::from_millis(100)).await.unwrap();
        tokio::time::sleep(Duration::from_millis(350)).await;
        // After ~3 ticks, snapshot should be Some.
