...
pub use meminfo::{parse_meminfo, read_meminfo, MeminfoReading};
pub use psi::{parse_psi, read_psi, PsiReading};
pub use swap::{parse_vmstat, read_vmstat, SwapState, VmstatReading};
...
pub fn snapshot(swap_state: &swap::SwapState) -> Result<MemorySnapshot, PressureReadError> {
    ...
    // Advance the delta tracker (caller can query SwapState::observe externally).
    ...
    // 6. Pressure classification (requires the struct to be mostly filled first).
    let mut snap = MemorySnapshot {
        ...
        psi_full_avg10,
        pgmajfault,
        pressure: Pressure::Green, // placeholder; overwritten immediately below
        captured_at_unix_ms,
    };
