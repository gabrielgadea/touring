...
fn extract_vmstat_field(line: &str, key: &'static str) -> Result<Option<u64>, PressureReadError> {
    // vmstat lines are exactly "key value" with a single space.
    let rest = match line.strip_prefix(key) {
        Some(r) => r,
        None => return Ok(None),
    };
    // The rest must start with whitespace (avoids "pgmajfault_something" false match).
...
impl SwapState {
    ...
    pub fn observe(&self, current_pgmajfault: u64) -> u64 {
        ...
        if prev == Self::UNSET || current_pgmajfault < prev {
            // First observation or counter wrap — return 0, not a spurious spike.
            0
        } else {
...
pgfault 12345678\n";
    ...
    fn swap_state_delta_computed_correctly() {
        let s = SwapState::new();
        s.observe(1000); // baseline
        assert_eq!(s.observe(1050), 50);
        assert_eq!(s.observe(1050), 0); // no change
    ...
    fn swap_state_counter_wrap_returns_zero() {
        let s = SwapState::new();
        s.observe(u64::MAX - 1); // near wrap
                                 // Simulate counter reset to 0 after wrap.
    ...
    fn live_read_vmstat_sanity() {
        ...
        // pgmajfault is cumulative since boot — must be >= 0 (always true for u64,
        // but we assert it's a plausible non-wrap value).
        let _ = r.pgmajfault; // just ensure it parses
    }
}
