...
pub fn parse_meminfo(content: &str) -> Result<MeminfoReading, PressureReadError> {
    // Field table: (prefix, slot-index).  Slot order: total, available, free,
    // swap_total, swap_free — matches the MeminfoReading field order below.
    ...
    // Require all five slots to be filled.
