...
fn extract_avg(line: &str, key: &'static str) -> Result<Option<f32>, PressureReadError> {
    // Locate "key=" in the token stream.
    let needle = {
        // Build "key=" as a stack string by iterating tokens — avoids heap alloc.
...
full avg10=8.90 avg60=3.45 avg300=0.12 total=77\n";
    ...
    fn parse_missing_full_line_returns_error() {
        ...
        assert!(
            matches!(err, Err(PressureReadError::MissingField { field, .. }) if field.contains("full")),
    ...
    fn parse_missing_avg60_returns_error() {
        ...
        let err = parse_psi(bad);
        assert!(
            matches!(err, Err(PressureReadError::MissingField { field, .. }) if field.contains("avg60")),
    ...
    fn live_read_psi_does_not_panic() {
        match read_psi() {
            ...
            Err(PressureReadError::PsiUnavailable) => {
                // Acceptable: kernel < 4.20 or psi=0.
            }
            Err(e) => {
