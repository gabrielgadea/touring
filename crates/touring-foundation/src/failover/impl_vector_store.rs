...
impl<P: VectorStoreProvider + 'static> Failover<P, VectorStoreBackup> for VectorStoreFailover<P> {
    ...
    async fn activate_backup(&mut self) -> Result<()> {
        ...
        // Pre-populate backup from primary to catch up
    ...
    async fn restore_to_primary(&mut self) -> Result<()> {
        ...
        // In a real implementation, this would reconfigure the primary
        // provider to use the backup data. For now, just log and return.
        tracing::info!(
...
mod tests {
    ...
    impl VectorStoreProvider for MockVectorProvider {
        ...
        fn fetch_vectors_since(&self, offset: u64) -> Result<Vec<EmbeddingVector>> {
            ...
            // Use insertion_order to determine which vectors to return,
            // avoiding HashMap iteration order issues
    ...
    async fn sync_incremental_vectors() {
        ...
        // Simulate new vectors added to primary
        failover
            .primary_provider
            .vector_count
