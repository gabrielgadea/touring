...
/// Returns `true` if `module_file` is a Rust source file eligible for wiring
/// inspection: extension `.rs`, NOT inside a benches/ or tests/ subtree
/// (with both relative-from-workspace and nested forms recognized).
///
/// Used as the gate for `register_pub_symbol` / `record_consumer` so that
/// Python scripts, Markdown plans, and Cargo benchmarks never pollute the
/// orphan-count diagnostic.
///
/// # Audit reference
///
...
///   `register_pub_symbol` accepted any extension.
#[must_use]
pub(crate) fn is_indexable_module_file(module_file: &str) -> bool {
    if !module_file.ends_with(".rs") {
        return false;
    }
    // Reject benches/* and tests/* in any position (leading or nested).
    // Both forms are checked because the wiring_map historically stored
    // both relative paths ("benches/src/foo.rs") and absolute paths
...
impl FileKnowledgeDB {
    ...
    /// - `non_rust_rows`: rows whose module_file does NOT end in `.rs`
    ///   (should be 0 after the eligibility gate at `register_pub_symbol`).
    ...
    pub fn orphan_symbols_for_module(
        ...
        let entries = stmt
            .query_map(params![canonical.as_ref()], |row| {
                Ok(WiringEntry {
                    module_file: row.get::<_, String>(0)?,
                    symbol_name: row.get::<_, String>(1)?,
                    symbol_kind: row.get::<_, String>(2)?,
