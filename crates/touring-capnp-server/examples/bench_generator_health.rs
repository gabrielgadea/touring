...
use touring_capnp_server::GeneratorHealthImpl;
...
async fn run_bench() {
    ...
    let stream = tokio::net::UnixStream::connect(&sock)
        .await
        .expect("connect");
    let mut rpc_system = util::make_rpc_system(stream, rpc_twoparty_capnp::Side::Client, None);
