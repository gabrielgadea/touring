...
use touring_foundation::TouringError;
use touring_foundation::moka_policies::{
    MokaCacheStats, build_knowledge_extended_cache, sample_stats,
};
...
mod query;
mod schema;
// A5 Path-A step-4: `impl KnowledgeSource for ThreadSafeKnowledgeDB` (orphan-rule
// correct here — ThreadSafeKnowledgeDB is local). Whole `knowledge` module is
// already `#[cfg(feature="knowledge")]`, which pulls in `dep:touring-intelligence`.
mod cognitive_bridge;
...
impl ThreadSafeKnowledgeDB {
    ...
    pub fn record_access(&self, file_path: &str, session_id: &str) -> Result<(), KnowledgeDbError> {
        self.with(|db| {
...
impl MvklKnowledgeBridge {
    ...
    pub fn add_relation(
        ...
        self.graph
            .lock()
            .expect("MvklKnowledgeBridge graph mutex poisoned")
            .add_edge(
