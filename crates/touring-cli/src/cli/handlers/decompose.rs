...
// ─── Utilitários compartilhados pelos submódulos ───────────────────────
//
// Vivem no pai porque `lifecycle` E `wayfinder` os consomem: pôr um no
// irmão faria um submódulo importar do outro, que é a dependência lateral
...
// `#[path]` explícito porque o crate declara este arquivo como
// `#[path = "cli/handlers/decompose.rs"] pub mod cli_handlers_decompose;`: o
// nome do MÓDULO não casa o do diretório, então o layout automático procura os
// submódulos em `src/cli_handlers_decompose/` e não os encontra. Mesma forma
// registrada no item 27(b) do CLAUDE.md — quem usa `#[path]` precisa nomear os
// filhos também.
#[path = "decompose/lifecycle.rs"]
mod lifecycle;
#[path = "decompose/wayfinder.rs"]
mod wayfinder;

// Re-exportados na raiz: o `mod tests` abaixo (e todo chamador externo)
// seguem vendo os handlers pelo mesmo caminho de antes da divisão.
pub use lifecycle::*;
pub use wayfinder::*;
