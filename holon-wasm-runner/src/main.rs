...
fn run(args: Args) -> anyhow::Result<String> {
    ...
    match args.cmd {
        ...
        Cmd::Invoke { capability, args } => {
            ...
            let raw = val_to_json(results.first());
            if let Some(unwrapped) = try_unwrap_invoke_response(&raw) {
                return Ok(unwrapped);
            }
            if let Some(formatted) = try_format_error(&raw) {
                return Ok(formatted);
            }
            Ok(raw)
        }
...
fn try_unwrap_invoke_response(json: &str) -> Option<String> {
    let json_val: serde_json::Value = serde_json::from_str(json).ok()?;
    let obj = json_val.as_object()?;
    // Unwrap Result<InvokeResponse>: extract stdout bytes → pretty JSON
    let ok_obj = obj.get("ok")?.as_object()?;
    let stdout_val = ok_obj.get("stdout")?.as_array()?;
    let bytes: Vec<u8> = stdout_val.iter().filter_map(|v| v.as_u64().and_then(|n| u8::try_from(n).ok())).collect();
    if bytes.is_empty() { return None; }
    let pretty = serde_json::from_slice::<serde_json::Value>(&bytes).ok()?;
    Some(serde_json::to_string_pretty(&pretty).unwrap_or_else(|_| json.to_string()))
}

fn try_format_error(json: &str) -> Option<String> {
    let json_val: serde_json::Value = serde_json::from_str(json).ok()?;
    let obj = json_val.as_object()?;
    let err_obj = obj.get("err")?.as_object()?;
    let mut compact = serde_json::Map::new();
    if let Some(case) = err_obj.get("case") { compact.insert("case".to_string(), case.clone()); }
    if let Some(payload) = err_obj.get("payload") { compact.insert("payload".to_string(), payload.clone()); }
    Some(serde_json::to_string(&compact).unwrap_or_else(|_| json.to_string()))
}
